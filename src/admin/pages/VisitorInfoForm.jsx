...
export default function VisitorInfoForm({ id, onBack, onNavigate }) {
  const [form, setForm] = useState({
    title: '', slug: '', excerpt: '', body: '', featured_image_url: '', status: 'draft',
    seo_title: '', seo_description: '', og_image_url: '', canonical_url: '', type: 'visitor_info',
    essential_offline: false, order: 0
  })
  ...
  const handleSubmit = async (status) => {
    ...
    setError(null)
    try {
      const payload = { ...form, status, type: 'visitor_info' }
  ...
  const handleDelete = async () => {
    if (!confirm('Are you sure you want to delete this visitor info entry? This cannot be undone.')) return
    setSaving(true)
    try {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">{id === 'new' ? 'New Visitor Info Entry' : 'Edit Visitor Info Entry'}</h1>
          <p className="text-slate-600">Manage visitor information</p>
        </div>
