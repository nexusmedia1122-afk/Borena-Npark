...
export default function AccommodationForm({ id, onBack, onNavigate }) {
  const [form, setForm] = useState({
    title: '', slug: '', excerpt: '', body: '', featured_image_url: '', status: 'draft',
    seo_title: '', seo_description: '', og_image_url: '', canonical_url: '', type: 'accommodation',
    price_range: '', contact_email: '', contact_phone: '', website: ''
  })
  ...
  const handleSubmit = async (status) => {
    ...
    setError(null)
    try {
      const payload = { ...form, status, type: 'accommodation' }
  ...
  const handleDelete = async () => {
    if (!confirm('Are you sure you want to delete this accommodation? This cannot be undone.')) return
    setSaving(true)
    try {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">{id === 'new' ? 'New Accommodation' : 'Edit Accommodation'}</h1>
          <p className="text-slate-600">Manage accommodation listing</p>
        </div>
      ...
      <div className="bg-white rounded-lg border border-slate-200 p-6 space-y-4">
        ...
        <div>
          ...
          <textarea value={form.body} onChange={(e) => setForm(f => ({ ...f, body: e.target.value }))} rows={3} className={inputClass} />
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className={labelClass}>Price Range</label>
            <input type="text" value={form.price_range} onChange={(e) => setForm(f => ({ ...f, price_range: e.target.value }))} className={inputClass} />
          </div>
          <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="tel" value={form.contact_phone} onChange={(e) => setForm(f => ({ ...f, contact_phone: e.target.value }))} className={inputClass} />
          </div>
        </div>
        <div>
          <label className={labelClass}>Website</label>
          <input type="url" value={form.website} onChange={(e) => setForm(f => ({ ...f, website: e.target.value }))} className={inputClass} />
        </div>
        <div>
