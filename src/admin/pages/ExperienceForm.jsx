...
export default function ExperienceForm({ id, onBack, onNavigate }) {
  const [form, setForm] = useState({
    title: '', slug: '', excerpt: '', body: '', featured_image_url: '', status: 'draft',
    seo_title: '', seo_description: '', og_image_url: '', canonical_url: '', type: 'experience'
  })
  ...
  const handleSubmit = async (status) => {
    ...
    setError(null)
    try {
      const payload = { ...form, status, type: 'experience' }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">{id === 'new' ? 'New Experience' : 'Edit Experience'}</h1>
          <p className="text-slate-600">Manage experience content</p>
        </div>
