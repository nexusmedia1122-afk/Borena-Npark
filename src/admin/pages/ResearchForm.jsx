...
export default function ResearchForm({ id, onBack, onNavigate }) {
  const [form, setForm] = useState({
    title: '', slug: '', excerpt: '', body: '', featured_image_url: '', status: 'draft',
    seo_title: '', seo_description: '', og_image_url: '', canonical_url: '', type: 'research'
  })
  ...
  const handleSubmit = async (status) => {
    ...
    setError(null)
    try {
      const payload = { ...form, status, type: 'research' }
  ...
  const handleDelete = async () => {
    if (!confirm('Are you sure you want to delete this research entry? This cannot be undone.')) return
    setSaving(true)
    try {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">{id === 'new' ? 'New Research Entry' : 'Edit Research Entry'}</h1>
          <p className="text-slate-600">Manage research publications</p>
        </div>
